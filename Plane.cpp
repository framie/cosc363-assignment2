...
bool Plane::isInside(glm::vec3 pt)
{
	...
	glm::vec3 point1 = glm::cross(b - a, pt - a);
	glm::vec3 point2 = glm::cross(c - b, pt - b);
	glm::vec3 point3 = glm::cross(d - c, pt - c);
	glm::vec3 point4 = glm::cross(a - d, pt - d);

	float point1n = glm::dot(point1, n);
	float point2n = glm::dot(point2, n);
	float point3n = glm::dot(point3, n);
	float point4n = glm::dot(point4, n);
	
	if (point1n > 0 && point2n > 0 && point3n > 0 && point4n > 0) 
	{
		return true;
	}
	else
...
{
	glm::vec3 n = glm::cross(b - a, d - a);   // (B - A) x (D - A)
	n = glm::normalize(n); 
